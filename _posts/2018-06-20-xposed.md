...
```
所以看到后面的代码不要奇怪，两种方法都是可以的。另外也可以先设置为new XC_MethodHook/XC_MethodReplacement的回调参数，然后再调用此参数，这种写法在循环hook中很常见（组合不定的参数列表和回调类参数，前者使用一个list或者array来存储），见[TrustMeAlready](https://github.com/ViRb3/TrustMeAlready/blob/master/app/src/main/java/com/virb3/trustmealready/Main.java)和[xposed新思路](https://bbs.pediy.com/thread-250616.htm)。  
