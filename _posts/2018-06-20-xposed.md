...
原因是Android5.0以下hook一个某个方法，而这个方法不在主dex，而存在分包dex。此时xposed会在没有加载分包dex的时候进行回调handleLoadPackage()，类加载器并没有加载分包里面的类 会导致 XposedHelpers.findAndHookMethod 抛出异常。见[Xposed遇到的坑](https://blog.csdn.net/qfanmingyiq/article/details/78323264) 另外四哥的文章也提到过很多次，所以需要hook Appliation的attach方法来解决多dex文件，如下图（其实看到后面会发现有一个更巧妙更简单的办法）
